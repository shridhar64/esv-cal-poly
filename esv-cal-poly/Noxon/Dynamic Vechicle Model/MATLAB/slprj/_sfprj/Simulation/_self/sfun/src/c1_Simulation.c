...
static void c1_c1_Simulation(SFc1_SimulationInstanceStruct *chartInstance);
static void compInitSubchartSimstructsFcn_c1_Simulation
  (SFc1_SimulationInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c1_machineNumber, uint32_T
  c1_chartNumber);
static real_T c1_mpower(SFc1_SimulationInstanceStruct *chartInstance, real_T
  c1_a);
static real_T c1_power(SFc1_SimulationInstanceStruct *chartInstance, real_T c1_a,
  real_T c1_b);
static void c1_eml_scalar_eg(SFc1_SimulationInstanceStruct *chartInstance);
static void c1_eml_error(SFc1_SimulationInstanceStruct *chartInstance);
static void c1_b_eml_scalar_eg(SFc1_SimulationInstanceStruct *chartInstance);
...
static real_T c1_b_mpower(SFc1_SimulationInstanceStruct *chartInstance, real_T
  c1_a);
...
static void c1_c_emlrt_marshallIn(SFc1_SimulationInstanceStruct *chartInstance,
  const mxArray *c1_pdot, const char_T *c1_name, real_T c1_y[10]);
static uint8_T c1_d_emlrt_marshallIn(SFc1_SimulationInstanceStruct
  ...
  chartInstance)
{
  ...
  c1_e_hoistedGlobal = chartInstance->c1_is_active_c1_Simulation;
  ...
  *chartInstance, const mxArray *c1_st)
{
  ...
  chartInstance->c1_is_active_c1_Simulation = c1_d_emlrt_marshallIn
...
static void sf_c1_Simulation(SFc1_SimulationInstanceStruct *chartInstance)
{
  ...
  c1_c1_Simulation(chartInstance);
  _sfEvent_ = c1_previousEvent;
  sf_debug_check_for_state_inconsistency(_SimulationMachineNumber_,
...
static uint8_T c1_d_emlrt_marshallIn(SFc1_SimulationInstanceStruct
  *chartInstance, const mxArray *c1_b_is_active_c1_Simulation,
  const char_T *c1_name)
{
  uint8_T c1_y;
  uint8_T c1_u0;
  sf_mex_import(c1_name, sf_mex_dup(c1_b_is_active_c1_Simulation), &c1_u0, 1, 3,
                0U, 0, 0U, 0);
  c1_y = c1_u0;
...
static mxArray *sf_get_sim_state_info_c1_Simulation(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x5'type','srcId','name','auxInfo'{{M[1],M[11],T\"Fout\",},{M[1],M[10],T\"acc_out\",},{M[1],M[5],T\"pdot\",},{M[1],M[9],T\"slp_ang\",},{M[8],M[0],T\"is_active_c1_Simulation\",}}"
  };
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        if (chartAlreadyPresent==0) {
          ...
          init_script_number_translation(_SimulationMachineNumber_,
            chartInstance->chartNumber);
          sf_debug_set_chart_disable_implicit_casting(_SimulationMachineNumber_,
            chartInstance->chartNumber,1);
          sf_debug_set_chart_event_thresholds(_SimulationMachineNumber_,
            chartInstance->chartNumber,
            0,
            0,
            0);
      ...
      sf_debug_reset_current_state_configuration(_SimulationMachineNumber_,
        chartInstance->chartNumber,chartInstance->instanceNumber);
    }
  }
}
...
static mxArray* sf_internal_get_sim_state_c1_Simulation(SimStruct* S)
{
  ...
  prhs[3] = sf_get_sim_state_info_c1_Simulation();/* state var info */
...
static void sf_internal_set_sim_state_c1_Simulation(SimStruct* S, const mxArray *
  st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
...
static mxArray* sf_opaque_get_sim_state_c1_Simulation(SimStruct* S)
{
...
{
  compInitSubchartSimstructsFcn_c1_Simulation((SFc1_SimulationInstanceStruct*)
    chartInstanceVar);
}
...
static void mdlSetWorkWidths_c1_Simulation(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"Simulation","Simulation",1);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"Simulation","Simulation",1,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"Simulation",
      "Simulation",1,"gatewayCannotBeInlinedMultipleTimes"));
    if (chartIsInlinable) {
      ssSetInputPortOptimOpts(S, 0, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 1, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 2, SS_REUSABLE_AND_LOCAL);
      ssSetInputPortOptimOpts(S, 3, SS_REUSABLE_AND_LOCAL);
      sf_mark_chart_expressionable_inputs(S,"Simulation","Simulation",1,4);
      sf_mark_chart_reusable_outputs(S,"Simulation","Simulation",1,4);
    }

    sf_set_rtw_dwork_info(S,"Simulation","Simulation",1);
...
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "Simulation", "Simulation",1);
...
{
  SFc1_SimulationInstanceStruct *chartInstance;
  chartInstance = (SFc1_SimulationInstanceStruct *)malloc(sizeof
