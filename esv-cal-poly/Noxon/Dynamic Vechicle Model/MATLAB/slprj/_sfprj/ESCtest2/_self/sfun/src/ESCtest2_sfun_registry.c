...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_ESCtest2_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_ESCtest2_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_ESCtest2_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_ESCtest2_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_ESCtest2_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "ESCtest2") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_ESCtest2_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_ESCtest2_method_dispatcher(SimStruct *S, unsigned int
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"ESCtest2")) {
    return(sf_ESCtest2_method_dispatcher(simstructPtr,chartFileNumber,specsCksum,
            method,data));
  }
...
extern void ESCtest2_terminator(void);
...
extern void ESCtest2_initializer(void);
extern void ESCtest2_register_exported_symbols(SimStruct* S);
extern void ESCtest2_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  ESCtest2_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      ESCtest2_debug_initialize();
    }

    ESCtest2_initializer();
  }
