...
%ctrl{ii}=mpt_simplify(ctrl1{ii},'Optimal');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Plot Trajectory for two initial Points..
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure
hold on;
grid on
...
xlabel(xlb)
ylabel(ylb)
print(gcf,'-depsc2',['Example13-1a.eps']);
...
xlabel(xlb)
ylabel(ylb)
print(gcf,'-depsc2',['Example13-1c.eps']);
...
hold on;
clear Options;
for i=sysStruct.xmin(1):0.5:sysStruct.xmax(1)
    for j=sysStruct.xmin(2):0.5:sysStruct.xmax(2)
        x0=[i,j];
        [X,U,Y,D,cost,trajectory,feasible]=mpt_computeTrajectory(ctrl1{1},x0,30);
        if feasible==1
            plot(i,j,'ks','MarkerFaceColor','k','MarkerSize',10)
        else
            plot(i,j,'ro','MarkerSize',10)
        end
    end
end
    Options.wire=1;
    Options.wirestyle='--';
%    Options.color=[ 0.5 0.5 1];
    plot(inv_set, Options)
...
xlabel(xlb)
ylabel(ylb)
print(gcf,'-depsc2',['Example13-1b.eps']);
