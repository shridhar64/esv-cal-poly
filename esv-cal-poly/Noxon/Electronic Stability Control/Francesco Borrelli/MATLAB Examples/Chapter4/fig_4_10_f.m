...
plot(G);
hold on
plot(D, 'k', struct('edgecolor', 'k', 'wire', 1, 'wirestyle', '-', 'linewidth', line_width));
plot(F, 'k', struct('edgecolor', 'k', 'wire', 1, 'wirestyle', '--', 'linewidth', line_width));
axis([-5 5 -5 5]);
% text in polytopes
text(-4.5, 0, 'F');
text(3.7, 3.7, 'D');
% arrows
arrow([-4.3 0.5], [-2.6 3.4], 'Width', line_width);
arrow([-4.3 -0.5], [-2.6 -3.4], 'Width', line_width);
arrow([3.5 3.5], [2.7 2.7], 'Width', line_width);
psfrag{1}{1} = 'F'; psfrag{1}{2} = '\mathcal{F}';
psfrag{2}{1} = 'D'; psfrag{2}{2} = '\mathcal{D}';
