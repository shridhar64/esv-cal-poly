...
#include "ESCtest4_sfun.h"
#include "c1_ESCtest4.h"
#include "mwmathutil.h"
#define CHARTINSTANCE_CHARTNUMBER      (chartInstance->chartNumber)
#define CHARTINSTANCE_INSTANCENUMBER   (chartInstance->instanceNumber)
#include "ESCtest4_sfun_debug_macros.h"
...
static void initialize_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance);
static void initialize_params_c1_ESCtest4(SFc1_ESCtest4InstanceStruct
  *chartInstance);
static void enable_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance);
static void disable_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance);
static void c1_update_debugger_state_c1_ESCtest4(SFc1_ESCtest4InstanceStruct
  *chartInstance);
static const mxArray *get_sim_state_c1_ESCtest4(SFc1_ESCtest4InstanceStruct
  *chartInstance);
static void set_sim_state_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance,
  const mxArray *c1_st);
static void finalize_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance);
static void sf_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance);
static void c1_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance);
static void compInitSubchartSimstructsFcn_c1_ESCtest4
  (SFc1_ESCtest4InstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c1_machineNumber, uint32_T
  c1_chartNumber);
static real_T c1_mpower(SFc1_ESCtest4InstanceStruct *chartInstance, real_T c1_a);
static real_T c1_power(SFc1_ESCtest4InstanceStruct *chartInstance, real_T c1_a,
  real_T c1_b);
static void c1_eml_scalar_eg(SFc1_ESCtest4InstanceStruct *chartInstance);
static void c1_eml_error(SFc1_ESCtest4InstanceStruct *chartInstance);
static void c1_b_eml_scalar_eg(SFc1_ESCtest4InstanceStruct *chartInstance);
static void c1_eml_xgemm(SFc1_ESCtest4InstanceStruct *chartInstance, real_T
  c1_A[9], real_T c1_B[9], real_T c1_C[9], real_T c1_b_C[9]);
static real_T c1_b_mpower(SFc1_ESCtest4InstanceStruct *chartInstance, real_T
  c1_a);
...
static void c1_emlrt_marshallIn(SFc1_ESCtest4InstanceStruct *chartInstance,
  const mxArray *c1_Fout, const char_T *c1_name, real_T c1_y[4]);
static void c1_b_emlrt_marshallIn(SFc1_ESCtest4InstanceStruct *chartInstance,
  const mxArray *c1_acc_out, const char_T *c1_name, real_T c1_y[5]);
static void c1_c_emlrt_marshallIn(SFc1_ESCtest4InstanceStruct *chartInstance,
  const mxArray *c1_pdot, const char_T *c1_name, real_T c1_y[10]);
static uint8_T c1_d_emlrt_marshallIn(SFc1_ESCtest4InstanceStruct *chartInstance,
  const mxArray *c1_b_is_active_c1_ESCtest4, const char_T *c1_name);
static void init_dsm_address_info(SFc1_ESCtest4InstanceStruct *chartInstance);

/* Function Definitions */
static void initialize_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance)
{
  _sfTime_ = (real_T)ssGetT(chartInstance->S);
  chartInstance->c1_is_active_c1_ESCtest4 = 0U;
}
  ...
  *chartInstance)
{
  ...
  c1_e_hoistedGlobal = chartInstance->c1_is_active_c1_ESCtest4;
  ...
  const mxArray *c1_st)
{
  ...
  chartInstance->c1_is_active_c1_ESCtest4 = c1_d_emlrt_marshallIn(chartInstance,
    sf_mex_dup(sf_mex_getcell(c1_u, 4)),
    "is_active_c1_ESCtest4");
  sf_mex_destroy(&c1_u);
  c1_update_debugger_state_c1_ESCtest4(chartInstance);
...
static void sf_c1_ESCtest4(SFc1_ESCtest4InstanceStruct *chartInstance)
{
  ...
  c1_c1_ESCtest4(chartInstance);
  _sfEvent_ = c1_previousEvent;
  sf_debug_check_for_state_inconsistency(_ESCtest4MachineNumber_,
...
static const mxArray *c1_sf_marshall(void *chartInstanceVoid, void *c1_u)
{
  ...
  const mxArray *c1_b_y = NULL;
  SFc1_ESCtest4InstanceStruct *chartInstance;
  chartInstance = (SFc1_ESCtest4InstanceStruct *)chartInstanceVoid;
  c1_y = NULL;
...
const mxArray *sf_c1_ESCtest4_get_eml_resolved_functions_info(void)
{
...
static uint8_T c1_d_emlrt_marshallIn(SFc1_ESCtest4InstanceStruct *chartInstance,
  const mxArray *c1_b_is_active_c1_ESCtest4, const
  char_T *c1_name)
{
  uint8_T c1_y;
  uint8_T c1_u0;
  sf_mex_import(c1_name, sf_mex_dup(c1_b_is_active_c1_ESCtest4), &c1_u0, 1, 3,
                0U, 0, 0U, 0);
  c1_y = c1_u0;
  sf_mex_destroy(&c1_b_is_active_c1_ESCtest4);
  return c1_y;
}
...
/* SFunction Glue Code */
void sf_c1_ESCtest4_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c1_ESCtest4_get_autoinheritance_info(void)
{
...
static mxArray *sf_get_sim_state_info_c1_ESCtest4(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x5'type','srcId','name','auxInfo'{{M[1],M[11],T\"Fout\",},{M[1],M[10],T\"acc_out\",},{M[1],M[5],T\"pdot\",},{M[1],M[9],T\"slp_ang\",},{M[8],M[0],T\"is_active_c1_ESCtest4\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 5, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c1_ESCtest4_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc1_ESCtest4InstanceStruct *chartInstance;
    chartInstance = (SFc1_ESCtest4InstanceStruct *) ((ChartInfoStruct *)
      (ssGetUserData(S)))->chartInstance;
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        unsigned int chartAlreadyPresent;
        chartAlreadyPresent = sf_debug_initialize_chart(_ESCtest4MachineNumber_,
          1,
          1,
          1,
          8,
        ...
        if (chartAlreadyPresent==0) {
          ...
          init_script_number_translation(_ESCtest4MachineNumber_,
            chartInstance->chartNumber);
          sf_debug_set_chart_disable_implicit_casting(_ESCtest4MachineNumber_,
            chartInstance->chartNumber,1);
          sf_debug_set_chart_event_thresholds(_ESCtest4MachineNumber_,
            chartInstance->chartNumber,
            0,
            0,
            0);
      ...
      sf_debug_reset_current_state_configuration(_ESCtest4MachineNumber_,
        chartInstance->chartNumber,chartInstance->instanceNumber);
    }
  }
}

static void sf_opaque_initialize_c1_ESCtest4(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc1_ESCtest4InstanceStruct*) chartInstanceVar)
    ->S,0);
  initialize_params_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*) chartInstanceVar);
  initialize_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c1_ESCtest4(void *chartInstanceVar)
{
  enable_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c1_ESCtest4(void *chartInstanceVar)
{
  disable_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c1_ESCtest4(void *chartInstanceVar)
{
  sf_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*) chartInstanceVar);
}

static mxArray* sf_internal_get_sim_state_c1_ESCtest4(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*)
    chartInfo->chartInstance);         /* raw sim ctx */
  prhs[3] = sf_get_sim_state_info_c1_ESCtest4();/* state var info */
...
static void sf_internal_set_sim_state_c1_ESCtest4(SimStruct* S, const mxArray
  *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  prhs[3] = (mxArray*) sf_get_sim_state_info_c1_ESCtest4();/* state var info */
  mxError = sf_mex_call_matlab(1, plhs, 4, prhs, "sfprivate");
  mxDestroyArray(prhs[0]);
  mxDestroyArray(prhs[1]);
  mxDestroyArray(prhs[2]);
  ...
  set_sim_state_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static mxArray* sf_opaque_get_sim_state_c1_ESCtest4(SimStruct* S)
{
  return sf_internal_get_sim_state_c1_ESCtest4(S);
}

static void sf_opaque_set_sim_state_c1_ESCtest4(SimStruct* S, const mxArray *st)
{
  sf_internal_set_sim_state_c1_ESCtest4(S, st);
}

static void sf_opaque_terminate_c1_ESCtest4(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc1_ESCtest4InstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*) chartInstanceVar);
...
{
  compInitSubchartSimstructsFcn_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c1_ESCtest4(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c1_ESCtest4((SFc1_ESCtest4InstanceStruct*)
      (((ChartInfoStruct *)ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c1_ESCtest4(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    int_T chartIsInlinable =
      (int_T)sf_is_chart_inlinable(S,"ESCtest4","ESCtest4",1);
    ssSetStateflowIsInlinable(S,chartIsInlinable);
    ssSetRTWCG(S,sf_rtw_info_uint_prop(S,"ESCtest4","ESCtest4",1,"RTWCG"));
    ssSetEnableFcnIsTrivial(S,1);
    ssSetDisableFcnIsTrivial(S,1);
    ssSetNotMultipleInlinable(S,sf_rtw_info_uint_prop(S,"ESCtest4","ESCtest4",1,
    ...
    if (chartIsInlinable) {
      ...
      sf_mark_chart_expressionable_inputs(S,"ESCtest4","ESCtest4",1,4);
      sf_mark_chart_reusable_outputs(S,"ESCtest4","ESCtest4",1,4);
    }

    sf_set_rtw_dwork_info(S,"ESCtest4","ESCtest4",1);
...
static void mdlRTW_c1_ESCtest4(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
    sf_write_symbol_mapping(S, "ESCtest4", "ESCtest4",1);
    ssWriteRTWStrParam(S, "StateflowChartType", "Embedded MATLAB");
  }
}

static void mdlStart_c1_ESCtest4(SimStruct *S)
{
  SFc1_ESCtest4InstanceStruct *chartInstance;
  chartInstance = (SFc1_ESCtest4InstanceStruct *)malloc(sizeof
    (SFc1_ESCtest4InstanceStruct));
  memset(chartInstance, 0, sizeof(SFc1_ESCtest4InstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c1_ESCtest4;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c1_ESCtest4;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c1_ESCtest4;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c1_ESCtest4;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c1_ESCtest4;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c1_ESCtest4;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c1_ESCtest4;
  chartInstance->chartInfo.getSimStateInfo = sf_get_sim_state_info_c1_ESCtest4;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c1_ESCtest4;
  chartInstance->chartInfo.mdlStart = mdlStart_c1_ESCtest4;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c1_ESCtest4;
...
void c1_ESCtest4_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c1_ESCtest4(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c1_ESCtest4(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c1_ESCtest4(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c1_ESCtest4_method_dispatcher.\n"
